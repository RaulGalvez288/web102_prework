...
let unfundedcond = numUnfunded > 1;
// create a string that explains the number of unfunded games using the ternary operator
const unfundedStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for 
${GAMES_JSON.length.toLocaleString('en-US')} games. Currently, ${numUnfunded} ${unfundedcond ? "games remain" : "game remains"} unfunded.
We need your help to fund ${unfundedcond ? "these" : "this"} amazing ${unfundedcond ? "games" : "game"}!`
...
firstElement.innerHTML = `<p>${first.name}</p>`;
firstGameContainer.appendChild(firstElement);

// do the same for the runner up item

const secondElement = document.createElement("div");
secondElement.innerHTML = `<p>${second.name}</p>`;
...
searchInput.addEventListener("input", e => {
    ...
    console.log(value)
    const searchFiltered = GAMES_JSON.filter(game => {
        return game.name.toLowerCase().includes(value);
    });
    addGamesToPage(searchFiltered);
});
